...
comet.pages.collection.helpers({
	path: function () {
...
if (Meteor.isServer) {
	Meteor.publish('pages', function(arg1, arg2) {
		arg1 = arg1 ? arg1 : {};
		return comet.pages.collection.find(arg1, arg2);
	});
}
 ...
 */
comet.pages.addTemplate = function(options, schema) {
	if (!options.template) {
	...
	var newSchema = comet.pages.getNewTemplateSchema(schema, newTemplate);
	newTemplate.schema = new SimpleSchema(newSchema);

	comet.pages.templates[newTemplate.template] = newTemplate;

	return newTemplate;
}

comet.pages.getNewTemplateSchema = function(schema, newTemplate) {
	return _.extend({
		title: {
			type: String,
			label: "Title",
